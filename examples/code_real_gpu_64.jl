...
function init_gpu_fft(N, M, L)
    ...
    tau1 = CUDA.zeros(ComplexF64, div(N, 2) + 1, M, L)
    tau2 = CUDA.zeros(ComplexF64, div(N, 2) + 1, M, L)
    tau3 = CUDA.zeros(ComplexF64, div(N, 2) + 1, M, L)
    tau4 = CUDA.zeros(ComplexF64, div(N, 2) + 1, M, L)
    tau5 = CUDA.zeros(ComplexF64, div(N, 2) + 1, M, L)
    tau6 = CUDA.zeros(ComplexF64, div(N, 2) + 1, M, L)

    return P, Pinv, xi1, xi2, xi3, tau1, tau2, tau3, tau4, tau5, tau6
end

function init_fields(N1, N2, N3)
    return CUDA.zeros(Float64, N1, N2, N3),
    CUDA.zeros(Float64, N1, N2, N3),
    CUDA.zeros(Float64, N1, N2, N3),
    CUDA.zeros(Float64, N1, N2, N3),
    CUDA.zeros(Float64, N1, N2, N3),
    CUDA.zeros(Float64, N1, N2, N3),
...
function main()
    ...
    r = CUDA.zeros(Float64, size(eps1))
    tol = 1e-30
    it_max = 50
